...
function _headers(callback) {
    ...
    const accessToken = _getAccessToken(function(accessToken) {
...
function refreshCredentials(callback) {
    ...
    _post(constants.loginPath, body, function(err, httpResponse, body) {
        logging.debug('credential response: ' + JSON.stringify(body))
        if (_.isNil(err)) {
            ...
            if ( !_.isNil(session) ) {
                ...
                if ( !_.isNil(token) && !_.isNil(userID) && !_.isNil(expirationDate)) {
                    ...
                    logging.error('Could not log in (missing body keys): ' + JSON.stringify(body))
                }
            }
        } else {
            logging.error('Could not log in: ' + JSON.stringify(body))
        }
...
function myDevices(callback) {
    ...
    _needsRenew(function(needsRenew) {
        ...
        _get(constants.myDevicesPath, null, function(err, httpResponse, body) {
            logging.debug('my devices response: ' + JSON.stringify(Object.keys(body)))
            if ( _.isNil(err) ) {
...
function queryDevice(device, callback) {
    ...
    _needsRenew(function(needsRenew) {
        ...
        _get(constants.devicesQueryBase + device + constants.deviceQuery, null, function(err, httpResponse, body) {
            logging.debug('my device response: ' + JSON.stringify(Object.keys(httpResponse)))
...
function poll() {
    ...
    _needsRenew(function(needsRenew) {
        ...
        if ( needsRenew ) {
            const accessToken = refreshCredentials(function(userID, token, expirationDate) {
